...
Goose usage is read locally from `sessions/sessions.db` in one global data directory - `$GOOSE_PATH_ROOT/data`, else `$XDG_DATA_HOME/goose`, else `~/.local/share/goose` (macOS resolves that same default as Linux, so there is no platform branch, and the Windows location is unverified). The database is WAL-mode, so it is read through the same copy-and-snapshot path as ZCode. `usage_ledger` is the only token source: one row per model request with its model on the row, so a mixed-model session prices request by request. `input_tokens` already contains the cached share, so the fresh part bills as input and the cached part at the cache rate, and `output_tokens` is gross because Goose persists no reasoning split anywhere. `cost`, `cost_source` and the `sessions.accumulated_*` / `total_tokens` columns are all ignored: the accumulated columns duplicate the ledger sum while `total_tokens` holds only the last request's snapshot, so reading both would count one request twice. Cost comes from the pricing DB, so a self-hosted model costs 0.00. Deleting a Goose session does remove its usage here, because the ledger rows leave the database with it. The Sessions tab lists top-level sessions only, one turn per ledger row, with active time taken from the request durations Goose records on its own assistant messages and a session Goose left unnamed titled by its first prompt; a scheduled recipe run currently lists as an ordinary session.

Roo Code usage is read locally from the extension's global storage: every `<globalStorage>/rooveterinaryinc.roo-cline/tasks/<taskId>/` that exists on the machine - the desktop trees for VS Code, Code - Insiders and VSCodium with their profiles, the `~/.vscode-server*` roots a WSL session writes, the Windows desktop tree under `/mnt/c` on WSL, `%APPDATA%` on Windows, `~/Library/Application Support` on macOS, and the `~/.vscode-mock/global-storage` root that `@roo-code/cli` writes. `TOKDASH_ROO_STORAGE_DIR` adds a relocated storage path to that list rather than replacing it. `ui_messages.json` is the token source: one entry per completed `api_req_started` request, which is exactly what Roo's own task total adds up. `tokensIn` is cache-inclusive and splits into input / cacheRead / cacheWrite, `tokensOut` is gross because Roo computes reasoning tokens but never persists them, and the recorded `cost` is ignored in favour of the pricing DB. A request that never finished keeps a pre-flight marker with no token keys, and a rewind truncates the file and appends a deletion marker, so neither is added back. The model is not in that file: it comes from Roo's own `<model>` tag in the sibling `api_conversation_history.json`, matched to each request. `tasks/_index.json` and the per-task totals in `history_item.json` are never read, because the index is written on a debounce and was measured four requests behind a live task. One blind spot is stated rather than hidden: deleting a task directory leaves its usage on the dashboard, because Tokdash keeps the rows of a file that went missing. The Sessions tab treats one task directory as one session, including after a resume, and delegated subtasks are billed but not listed.
