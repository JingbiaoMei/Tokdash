...
build_number="${COMPANION_BUILD_NUMBER:-1}"
...
if [[ ! "$build_number" =~ ^[0-9]+$ ]]; then
  echo "COMPANION_BUILD_NUMBER must be a positive integer" >&2
  exit 1
fi
...
dmg_path="$output_dir/TokdashCompanion-$version-macos-universal-$release_suffix.dmg"
rm -f "$dmg_path"
hdiutil create \
...
if [[ -z "$signing_identity" ]]; then
  echo "WARNING: artifact is unsigned and intended only for an explicit prerelease." >&2
fi
