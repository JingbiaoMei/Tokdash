...
- Goose usage tracking. Tokdash reads `usage_ledger` in Goose's `sessions/sessions.db` (under `$GOOSE_PATH_ROOT/data`, `$XDG_DATA_HOME/goose`, or `~/.local/share/goose`) and takes one entry per model request, with the model on the row, so a session that switched models mid-way prices request by request instead of at whichever model the session happened to end on. `input_tokens` is cache-inclusive and splits into fresh input plus cache read, and `output_tokens` is billed whole because Goose persists no reasoning split in any table. The database is WAL, so it is read through the same copy-and-snapshot path as ZCode rather than opened in place, and cost comes from the pricing DB alone, which is why a self-hosted model id reads `0.00` rather than a made-up number. Goose's own aggregates are deliberately not read: `sessions.accumulated_input_tokens` / `accumulated_output_tokens` duplicate the ledger sum exactly, while `sessions.total_tokens` holds only the last request's snapshot, so summing the session columns undercounts every multi-request session and summing both sets of columns counts the last request twice. Compaction requests are real billed requests and stay, flagged so they can be told apart. Sessions lists top-level sessions with one turn per ledger row, and takes active time from the `elapsedMs` Goose stamps on its own assistant messages, matched by request order; because that pairing is positional, it is checked rather than trusted - a session whose billed rows and duration-bearing messages disagree in count keeps the existing capped inter-event-gap estimate instead of charging each turn its neighbour's duration. Deleting a session in Goose removes its usage here, since its ledger rows leave the database with it. See the SUPPORTED_CLIENTS entry for the full accounting rules. (#103)
- Roo Code usage tracking, for the VS Code extension and `@roo-code/cli` alike. Every existing `<globalStorage>/rooveterinaryinc.roo-cline/tasks/<taskId>/` is read - VS Code, Code - Insiders and VSCodium desktop trees including profiles, `~/.vscode-server*` on Linux and WSL, the `/mnt/c/Users/*` desktop tree on WSL only, `%APPDATA%` on Windows, `~/Library/Application Support/Code*` on macOS, and the `~/.vscode-mock/global-storage` root the CLI writes - with roots gated on the current OS kind so a Linux process never enumerates a Windows user tree, and `TOKDASH_ROO_STORAGE_DIR` adding a relocated `customStoragePath` to that list rather than replacing it. One entry is taken per completed `say:"api_req_started"` message, which is exactly what Roo's own task total sums: `tokensIn` is cache-inclusive and splits through the same helper Cline uses, `tokensOut` is gross because Roo computes reasoning tokens and never persists them, and the payload's `cost` is ignored so a self-hosted endpoint prices at `0.00` like any unlisted id. Task files are re-read whole rather than tailed, because a rewind truncates the file in place and an append-only reader would keep billing rows Roo has already removed. Task discovery is a directory glob, not `tasks/_index.json`: the index is written on a debounce and was measured four requests behind its own task, and had no entry at all for an aborted task that did carry a billable row. Roo persists no model field, so each request's model is paired with Roo's own `<model>` header in the sibling `api_conversation_history.json`, nearest within half a second, else the model in force, else `unknown`. Sessions is one entry per task directory including resumes, which append in place; a task carrying `parentTaskId` is a delegated subtask, billed but not listed. Deleting a task directory leaves its usage on the dashboard: the usage index is durable by default, so a task directory that disappears keeps the rows already collected for it. (#103)
