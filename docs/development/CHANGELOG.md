...
- A second Claude Code subscription now reaches the Quota tab. Claude Code keeps one sign-in per config directory, so a profile run as `CLAUDE_CONFIG_DIR=~/.claude-academic claude` has its own windows, and only the default install's were ever polled. Tokdash now reads every `~/.claude*` install with its own `.credentials.json` (under the same `quota.credential_scan` consent) and polls each one separately. The Claude card groups the windows by install under the directory name it was set up with (`academic`), the consumption chart plots `Claude-academic 5-hour` beside `Claude 5-hour`, and an install whose sign-in expired carries its own notice instead of hiding a working install's numbers. Two directories holding the same sign-in report once, and `TOKDASH_CLAUDE_PROFILES` names a path-separated list of installs for machines where they live outside the home directory. Usage totals needed no change: session logs under every `~/.claude*` install have been counted for some time; this is the quota side catching up.
