...
- Goose mark: the official Goose desktop app icon (`ui/desktop/src/images/icon.svg`) from the [Goose repository](https://github.com/aaif-goose/goose), a black goose on a white rounded tile. The packaged copy is byte-identical; keep it that way. Goose's own docs theme sets its primary color to black, so the entry uses `#111111`, and it carries `darkInvert` because the tile inverts to a black square with a light goose rather than sitting as a white hole in a dark row.
- Roo Code mark: the official Roo Code extension icon (`src/assets/icons/icon.svg`) from the [Roo Code repository](https://github.com/RooCodeInc/Roo-Code), a black kangaroo on a transparent 96x96 canvas. The packaged copy is byte-identical; keep it that way. Roo ships its logo monochrome (its docs offer only `-dark` and `-white` lockups), so the entry uses the same near-black as the Zed and Pi marks with `darkInvert`.
